...
**[Imagens do Paciente — DEPLOYADO e NO AR] (2026-08-07)** — Painel de **imagens por paciente** (até 5, no nível do paciente, não por consulta), no rodapé da ficha (slot `rodape` novo em `Avaliacoes.jsx`, entre a Observação e o botão Salvar). Spec/tasks em `.specs/features/imagens-paciente/` (IMG-01..09). **Arquitetura:** binário no **S3** (bucket privado `clinica-pilates-patientimagesbucket-ff6veht0bouu`), a Lambda só assina **URLs pré-assinadas** (5 min) — o binário nunca passa pela API. Item `SK=IMAGE#<id>` sob a PK do paciente. Upload em **2 fases** (sem órfãos): `POST` valida tipo+limite → presigned PUT; browser sobe no S3; `PUT` confirma via `head_object` → grava. `DELETE` apaga do S3+item. **Commit:** `7519f83` na `feat/auth-cognito`. **Deploy feito (2026-08-07):** changeset **aditivo** — só `+ PatientImagesBucket` e Modify (sem Replacement) em Lambda/IAM/HttpApi; **`ClinicaTable` e o Cognito NÃO foram tocados** (dados do cliente real intactos, ninguém deslogado). **Verificado na stack real:** `/health` **200** (cold start OK com os módulos novos — prova que não há erro de import), `/pacientes` sem token **401**, `IMAGES_BUCKET` injetado na Lambda, bucket com BlockPublicAccess **total** e CORS correto. `pytest` **219 verde**, `npm run build` OK. ⏭️ **Falta:** (1) **teste end-to-end no browser** com login real (a sessão automatizada não tem a senha) — subir imagem, ver miniatura, remover; (2) **publicar o front no CloudFront** (o painel de imagens ainda NÃO está no ar para o cliente — o back já está). Testar antes em `localhost:5173` (`npm run dev`) contra a API real. Detalhes ⬇️.
