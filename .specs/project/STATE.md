...
**[Imagens do Paciente — IMPLEMENTADO no código, NÃO DEPLOYADO ainda] (2026-08-03)** — Painel de **imagens por paciente** (até 5, no nível do paciente, não por consulta) na ficha, abaixo das avaliações. Spec/tasks em `.specs/features/imagens-paciente/` (IMG-01..09). **Arquitetura:** binário no **S3** (bucket privado novo `PatientImagesBucket`, CORS p/ pilatesone.com.br + localhost), a Lambda só assina **URLs pré-assinadas** (o binário nunca passa pela API). Item `SK=IMAGE#<id>` sob a PK do paciente (metadado: key/contentType/criadoEm). Upload em **2 fases** (sem órfãos): `POST` valida tipo+limite → presigned PUT; browser sobe no S3; `PUT` confirma via `head_object` → grava. `GET` lista com presigned GET; `DELETE` apaga do S3+item. **Back:** `template.yaml` (+bucket +IAM PutObject/GetObject/DeleteObject +env IMAGES_BUCKET), `src/app/s3_images.py`, `schemas_imagem.py`, `repository_imagem.py`, `routers/imagens.py`, fiado no `main.py`. **Front:** `imagensApi`+`uploadParaS3` em `api.js`, `components/ImagensPaciente.jsx` (grid de miniaturas, +Adicionar até 5, remover, validação tipo/5MB), fiado no fim da ficha em `Pacientes.jsx`, CSS em `App.css`. **Verificado local:** `pytest` **219 verde** (+18: 4 s3 helper, 5 repo, 9 endpoints — fluxo 2 fases c/ moto S3), `npm run build` OK, `sam validate --lint` OK. ⚠️ **NÃO COMMITADO / NÃO DEPLOYADO.** ⏭️ **Ao retomar:** deploy é **aditivo e seguro** (não afeta pacientes/avaliações/sessões). Testar "local first" = `sam build --use-container && sam deploy` (cria o bucket) → `frontend/.env` c/ `VITE_COGNITO_CLIENT_ID=6cke2rskmcq3ogim7c7ns4qqbn` → `cd frontend && npm run dev` → testar a ficha em localhost:5173 contra a API real (front ainda NÃO publicado no CloudFront). Só depois publicar o front. Detalhes ⬇️.
