...
**Last Updated:** 2026-07-27 — **Registro de Sessões (Aula de Pilates): COMPLETO (back + front) e NO AR.** Backend deployado (S1 schemas + S2 repo + S3 router, 44 testes novos → suíte **180 verde**; smoke-test público OK). Front: aba **"Pilates"** publicada no CloudFront (bundle `index-AxrDyLup.js`) — busca aluno → combo de aparelhos + chips de treino por aparelho → observação/profissional → salvar; histórico datado com consultar/editar/remover. Verificado ponta a ponta (Playwright com API mockada; browser do ambiente não alcança a API real). Feature `registro-sessoes` **concluída**. Commitado e **pushado** (`origin/main`).
